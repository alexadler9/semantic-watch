...
- проверка доступа выполняется до загрузки URL и LLM-вызовов;
- `/list`, `/check` и `/stop` проверяют владельца;
- количество активных задач ограничено для каждого пользователя.
...
- AI вызывается только при создании правила и при изменении hash страницы;
...
- совпадение принимается только при `conditionMatched=true;
